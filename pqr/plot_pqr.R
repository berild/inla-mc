...
library(ggpubr)
source("./genFuncs.R")
load("./pqr/pqr-gammarw2-amis.Rdata")
...
ggarrange(p5,p7,p6,ncol=1, nrow=3, common.legend = T,legend="bottom",labels = c("a", "b","c"),font.label = list(size = 10))
...
p5
p6 <- ggplot() + 
  geom_contour(data = mcmc_w_inla_mod$eta_joint_kern, aes(x = x, y = y, z = z),bins = 6,color = "black") + 
  theme_bw() +
  coord_cartesian(xlim = c(12.5,15.0), y = c(-0.0165,-0.012)) + 
  labs(x=expression(alpha),y=expression(beta))
p6
p7 <- ggplot() + 
  geom_contour(data = is_w_inla_mod$eta_joint_kern, aes(x = x, y = y, z = z),bins = 6, color = "black") + 
  theme_bw() +
  coord_cartesian(xlim = c(12.5,15.0), y = c(-0.0165,-0.012)) + 
  labs(x=expression(alpha),y=expression(beta))
p7

ggarrange(p5,p7,p6,ncol=1, nrow=3,labels = c("a", "b","c"),font.label = list(size = 10))
