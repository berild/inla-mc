...
calc.theta <- function(curr,i_tot){
  ...
  theta = list(numeric(ncol(eta)),matrix(NA, ncol= ncol(eta), nrow = ncol(eta)))
  for (i in seq(ncol(eta))){
    theta[[1]][i] = sum(eta[1:i_tot,i]*weight[1:i_tot])/sum(weight[1:i_tot])
  }
  for (i in seq(ncol(eta))){
    for (j in seq(i,ncol(eta))){
      theta[[2]][i,j] = theta[[2]][j,i] = sum(weight[1:i_tot]*(eta[1:i_tot,i]-theta[[1]][i])*
...
fit.marginals <- function(ws,margs,len = 400,low = 0.25,high = 0.75){
  xmin <- quantile(apply(margs[[1]],1,function(X){min(X)}),low)
  xmax <- quantile(apply(margs[[1]],1,function(X){max(X)}),high)
  ...
  for (i in seq(nrow(margs[[1]]))){
    tryCatch({
      res =  ws[i]*INLA::inla.dmarginal(xx, list(x = margs[[1]][i,], y = margs[[2]][i,]))
    },error=function(e){
...
fit.marginals2 <- function(ws,margs,len = 400){
  xmin = min(margs[[1]][which.max(ws),])
  xmax = max(margs[[1]][which.max(ws),])
  xx <- seq(xmin,xmax,len=len)
  ws = ws/sum(ws)
  marg = numeric(len)
  res = 0
